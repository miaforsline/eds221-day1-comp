...
```{r}
#read.csv (may convert character -> factor) vs read_csv (expects certain things based on the first 1000 rows)
#data.table::fread() = good for reading in giant datasets 
#check here() working directory using `here()` 
#manually include NA values 
...
# What is this %>% (command + shift + M on a Mac) thing? Meet the pipe operator! Also, as of a couple months ago, |> is a native pipe operator (versus %>%, which comes along with the tidyverse). 
# %>% = "and then" = sequential steps = write code in an organization/order that makes more sense to us = avoid nested functions 
#toolik_weather %>% clean_names() AKA clean_names(toolik_weather)

# Now what are the column names? lowercase_snakecase 
```
...
```{r}
#month stored as numeric by default 
...
```{r}
#here() can read in files or send files somewhere else 
